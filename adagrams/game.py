...
def draw_letters(): 
    ...
    chosen_letters = random.sample(available_letters, 10)  
...
def uses_available_letters(word, letter_bank):
    ...
    for letter in word.upper():
        #if letter in bank copy remove the letter, else return false and break
...
def get_highest_word_score(word_list):
    ...
    best_word = min(word_scores_dict[highest_score], key = lambda word: len(word))
